...
load('C:/Users/scwatson/Stella Self Dropbox/Stella Watson/Synced Files/Research/GMRF with Unknown Structures/Quantile Regression Spatial Smoother Paper/Data Application/PMdataframes2.RData')
...
sparse.W <-function(g.s){
  ...
  ind = int.seq%%g.s ==1 
  int.seq = int.seq[!ind]
  col.ind = unlist(lapply(as.matrix(int.seq),col.func,gd = g.s))
  row.ind = unlist(lapply(as.matrix(int.seq),row.func))
...
z.samp <-function(i,lambda.i,chi.i,psi.i){
...
###Format the Data
main.df = big.pm.lst.summ[[1]]
for(i in 2:length(big.pm.lst.summ)){
  main.df = rbind(main.df,big.pm.lst.summ[[i]])
}
...
W.psi = sparse.W.time(n.basis.re,n.basis.re)
...
for(g in 1:G){
  ...
  for(i in 1:P){
    ...
    for(j in 1:P){
      ...
      phi.g[,i] = mvn.samp(phi.mean,phi.sig)
  ...
  for(j in 1:yr){
    ...
    psi.array.g[,j] = mvn.samp(psi.mean,psi.sig)
  }
  psi.g = c()
  for(j in 1:yr){
  ...
  Z.g = apply(matrix(seq(1,n),n,1),1,z.samp,lambda.i = 1/2, chi.i = Z.chi, psi.i = 2 + theta^2/(lambda^2))
  Z.inv.g = 1/Z.g
  Z.inv.g.D = sparseMatrix(i = seq(1,n),j = seq(1,n),x = Z.inv.g)
  psi.array[,g] = psi.g
...
###Make the Maps
#path = "C:/Users/Travis/Stella Self Dropbox/Stella Watson/Synced Files/Research/GMRF with Unknown Structures/Quantile Regression Spatial Smoother Paper/Spline Redo/Data Apps/Summer50/3d car dg 3 AND dg 3 SVC/"
library(rgdal)
library(raster)
...
for(p in 1:P){
  ...
  windows(width = 10)
  pdf(paste0(path,"RCSummer50 ",plot.names[p],'.pdf'))
...
#   base = disaggregate(base,10)
#   windows(width = 10)
#   pdf(paste0(path,"Summer50 ",plot.names[p],'.pdf'))
#   plot(m1,main = plot.names[p])
#   p1 = mask(base$val.neg,m1)
#   p2 = mask(base$val.pos,m1)
#   p3 = mask(base$val,m1)
...
# }
windows(width = 3)
pdf(paste0(path,"SummerR50 Legend.pdf"))
...
for(p in 1:P){
  ...
  base = disaggregate(base,10)
  windows(width = 10)
  pdf(paste0(path,"Sig Summer50 ",plot.names[p],'.pdf'))
  plot(m1,main = plot.names[p])
  p1 = mask(base$val.neg,m1)
  p2 = mask(base$val.pos,m1)
  p3 = mask(base$val,m1)
