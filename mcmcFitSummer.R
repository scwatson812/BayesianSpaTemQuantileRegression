...
mcmcFitSummer<-function(G, burn,r,path,m1){
  ...
  ###Lobally Standardize Covariates
  for(s in 1:length(station.list)){
    s.ind = main.df$stnID == station.list[s]
    main.df$PM[s.ind] = (main.df$PM[s.ind]-mean(main.df$PM[s.ind]))/sd(main.df$PM[s.ind])
    for(j in 11:21){
      main.df[s.ind,j] = (main.df[s.ind,j] - mean(main.df[s.ind,j]))/sd(main.df[s.ind,j])
    }
  }
  ...
  for(p in 1:P){
    ...
    windows(width = 10)
    pdf(paste0(path,"Summer ",plot.names[p],'.pdf'))
  ...
  windows(width = 3)
  pdf(paste0(path,"Summer Legend.pdf"))
  image(z = t(matrix(c(seq(from = min(phi.hat),to = 0, length.out = 9),seq(from = seq(from = 0, to = max(phi.hat),length.out = 9)[2], to = max(phi.hat),length.out = 9)),1,18)), col = c(brewer.pal(9,"Blues")[seq(from = 9, to = 1)],brewer.pal(9,"Reds")),axes = FALSE,breaks = c(seq(from = min(phi.hat),to = 0, length.out = 10),seq(from = seq(from = 0, to = max(phi.hat),length.out = 9)[2], to = max(phi.hat),length.out = 9)))
